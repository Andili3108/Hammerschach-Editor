...
function updateOnlineActionButtons(){
  ...
  if(roomLobbyBtn){
    ...
    if(embeddedToolActive()){
      const activeName=learningToolActive?'Schach lernen':(mateSchoolToolActive?'Mattbilder-Schule':(leagueStandingsToolActive?'Ligatabellen':(readerToolActive?'Partienarchiv':(tvToolActive?'Gamer-TV':(openingsToolActive?'Eröffnungsschule':(schachlaborToolActive?'Schachlabor':(trainerToolActive?'Taktiktrainer':'Analyzer')))))));
