...
function updateOnlineActionButtons(){
  ...
  if(roomLobbyBtn){
    ...
    if(embeddedToolActive()){
      const activeName=impulsesToolActive?'Trainingsimpulse':mediathekToolActive?'Mediathek':learningToolActive?'Videokurse':(mateSchoolToolActive?'Mattbilder-Schule':(leagueStandingsToolActive?'Ligatabellen':(tournamentReportToolActive?'Turnierbericht':(readerToolActive?'Partienarchiv':(tvToolActive?'Gamer-TV':(openingsToolActive?'Eröffnungsschule':(schachlaborToolActive?'Schachlabor':(trainerToolActive?'Taktiktrainer':'Analyzer'))))))));
