...
(function(){
 ...
 let appliedBoard='',appliedPieces='',appliedScheme=null,appliedSound=null;
 function apply(){const p=P.snapshot();if(p.board!==appliedBoard){appliedBoard=p.board;applyBoardColorPreset(p.board,false,true);}if(p.pieces!==appliedPieces){appliedPieces=p.pieces;applyPieceSetPreset(p.pieces,false,true,true);}if(p.scheme!==appliedScheme){appliedScheme=p.scheme;setColorScheme(p.scheme);}if(p.sound!==appliedSound){appliedSound=p.sound;setSoundEnabled(p.sound);}if(!p.premoves&&queuedPremove)cancelQueuedPremove();updateSidePanelLayout();refresh();broadcast();}
