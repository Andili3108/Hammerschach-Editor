...
(function initialiseMobileNavigation(){
  ...
  function syncStatus(){
    ...
    if(testContext){
      ...
      testContext.textContent = root.classList.contains('mediathek-tool-active') ? 'Mediathek' : learning ? 'Schachschule' : (trainer ? 'Trainer' : (mateSchool ? 'Mattbilder' : (leagueStandings ? 'Ergebnisdienst' : (tournamentReport ? 'Turnierbericht' : (reader ? 'Partienarchiv' : (tv ? 'Gamer-TV' : (workshop ? 'Werkstatt' : (roomContext() ? 'Partie' : (root.classList.contains('member-lobby-view') ? 'Lobby' : 'Gamer')))))))));
    }
  }
